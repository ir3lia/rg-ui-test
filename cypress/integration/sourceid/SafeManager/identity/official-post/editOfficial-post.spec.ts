...
describe('编辑正式岗位', () => {
  ...
  //用例：编辑正式岗位
    it('编辑正式岗位', () => {
     ...
     //点击编辑按钮
     cy.get('#content-wrapper > section > article > h3 > i').click()
     //清除岗位编号输入框内容
     cy.get('#content-wrapper > section > article > app-form-build > form > section:nth-child(1) > div:nth-child(2) > nz-form-item > nz-form-control > div > span > input').clear()
     //岗位编号输入框内容更改为zhengke0003
     cy.get('#content-wrapper > section > article > app-form-build > form > section:nth-child(1) > div:nth-child(2) > nz-form-item > nz-form-control > div > span > input')
     .type('zhengke0003')
     //点击保存
     cy.get('#content-wrapper > section > article > app-form-build > form > div > article > button.ant-btn.ant-btn-primary').click()
